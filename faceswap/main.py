...
def foo():
    ...
    parser.add_argument('--source_img_path', type=str, default='data/source.png')
    parser.add_argument('--target_img_path', type=str, default='data/target.png')
...
def image(source: UploadFile = File(...), target: UploadFile = File(...)):
    location = [f"data/source.png", f"data/target.png"]
    for file_location, uploaded_file in zip(location, [source, target]):
        with open(file_location, "wb+") as file_object:
            file_object.write(uploaded_file.file.read())
    foo()
    return FileResponse("results/target.png")
